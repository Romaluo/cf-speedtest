...
| 能力维度 | CF-SpeedTest | 传统 Cloudflare IP 工具 |
|---------|-------------|------------------------|
| **IP 归属地验证** | xdb 初筛 + cdn-cgi/trace 精准纠错，消除 90%+ 误判 | 仅依赖 xdb 静态库，误判率高 |
| **IP 源策略** | 官方 CIDR + 自定义 URL + CIDR 动态权重采样 | 仅官方 CIDR 随机采样 |
| **测速流程** | TCP 预筛选 → 国家筛选 → 纠错验证 → 测速 → 评分 → 入库 | 直接测速，无预筛选和纠错 |
| **质量闸门** | 4 条硬性准入规则 + 可配置评分阈值 | 仅评分筛选，无硬性规则 |
| **资源效率** | 增量测速（24h 有效期）+ 批次降级重试 + 流式入库 | 全量重测，资源浪费严重 |
| **管理方式** | 内置 Web Dashboard 全流程可视化管理 | 纯命令行操作 |
| **自动化** | systemd 服务 + 定时采集 + 定时推送 + 状态持久化 | 需手动 cron 配合 |
| **推送目标** | Cloudflare DNS + GitHub + WxPusher 微信通知 | 单一目标 |

### 技术实现创新

1. **两级 IP 归属地验证体系**：第一级使用 ip2region.xdb 快速初筛（毫秒级），第二级通过 cdn-cgi/trace 端点进行精准纠错验证，验证失败的 IP 直接剔除（不进入测速流程），验证结果持久化到纠错层，下次无需重复验证。

2. **CIDR 动态权重采样**：系统自动统计每个 CIDR 段通过筛选的 IP 数量，形成动态权重分布，后续采集时优先从高权重段采样，显著提高优质 IP 命中率。

3. **自适应批次降级重试**：当批次失败率超过 30% 时，自动降低并发数进行二次重试，在不稳定网络环境下仍能获取有效结果。

4. **后台模式状态持久化**：通过 Web UI 切换的后台模式状态自动写入 config.yaml，系统重启后自动恢复定时任务，无需重新配置。

5. **系统级资源清理框架**：每次任务完成后自动执行 GC 回收、临时文件清理、残留进程终止，支持按任务类型配置差异化清理策略。

6. **纯 Go 零 CGO 依赖**：所有功能（包括 SQLite、Web 服务器、IP 归属地查询）均使用纯 Go 实现，一处编译全平台运行。

### 解决的关键问题

- **Cloudflare IP 归属地误判**：xdb 静态库对动态 IP 的归属地识别准确率不足，通过 cdn-cgi/trace 实时验证进行纠正
- **大规模 IP 采集资源浪费**：通过有效期机制实现增量测速，24 小时内已测 IP 直接跳过，避免重复测速
- **后台任务状态丢失**：通过配置文件持久化 daemon_mode，重启后自动恢复定时运行状态
- **数据库无限膨胀**：末位淘汰 + 过期清理双重机制，自动管理数据生命周期
- **多源 IP 冲突与去重**：官方 CIDR 与自定义 URL 自动合并去重，统一处理
